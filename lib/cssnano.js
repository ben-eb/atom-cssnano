...
module.exports = {
    ...
    process: function () {
        ...
        cssnano.process(text, opts).then(function (result) {
            ...
            } else if (atom.config.get('cssnano.minifyToFile')) {
              var filePath = editor.getPath();
              var filenamePath = filePath.replace('.css', '.min.css');
              fs.writeFile(filenamePath, result.css, function (err) {
                if (err) {
